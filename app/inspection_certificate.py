...
def update_inspection(
    ...
    return RedirectResponse(
        url=shipment_detail_redirect_url(current.get("shipment_no", ""), account_id, "/inspection-list"), status_code=303,
    )
...
def create_inspection_pdf(request: Request, payload: dict = Body(...)):
    ...
    def draw_header():
        ...
        pdf.drawString(45, height - 118, fit_text(f"Inspection No: {inspection_no}", 505, TP_UNICODE_BOLD, 10))
        ...
        pdf.drawString(45, height - 304, fit_text(f"Transport Details: {transport_details}", 505, TP_UNICODE_BOLD, 8))
        pdf.drawString(45, height - 321, fit_text(f"Port of Loading: {port_of_loading}", 235, TP_UNICODE_BOLD, 8))
        pdf.drawString(300, height - 321, fit_text(f"Port of Discharge: {port_of_discharge}", 250, TP_UNICODE_BOLD, 8))
    ...
    pdf.setFont(TP_UNICODE, 9)
    pdf.drawString(60, result_top - 45, fit_text(inspection_result, 455, TP_UNICODE, 9))
    if remarks:
        pdf.drawString(60, result_top - 65, fit_text(f"Remarks: {remarks}", 455, TP_UNICODE, 9))
