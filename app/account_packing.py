...
def ensure_legacy_packing_ownership(packing_file, users_file):
    packing_file = Path(packing_file)
    records = load_json_strict(packing_file, [], list)
    if not any(
    ...
    if len(account_ids) != 1:
        raise StorageValidationError(
            "Legacy Packing List ownership cannot be determined safely."
        )
    ...
    return locked_json_mutation(packing_file, [], assign_owner, list)


def public_packing(record):
