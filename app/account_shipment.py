...
def ensure_legacy_shipment_ownership(shipment_file, users_file):
    shipment_file = Path(shipment_file)
    records = load_json_strict(shipment_file, [], list)
    if not any(
    ...
    if len(account_ids) != 1:
        raise StorageValidationError(
            "Legacy Shipment ownership cannot be determined safely."
        )
    ...
    return locked_json_mutation(shipment_file, [], assign_owner, list)


def public_shipment(record):
