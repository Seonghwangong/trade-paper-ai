...
def ensure_legacy_shipping_instruction_ownership(si_file, users_file):
    si_file = Path(si_file)
    records = load_json_strict(si_file, [], list)
    if not any(
    ...
    if len(account_ids) != 1:
        raise StorageValidationError(
            "Legacy Shipping Instruction ownership cannot be determined safely."
        )
    ...
    return locked_json_mutation(si_file, [], assign_owner, list)


def public_shipping_instruction(record):
