...
from app import paddle_live_payment_method as payment_method
...
def status_for(account, *, now=None):
    ...
    with store.connect() as db:
        ...
        decision = store.decision_in_transaction(db, row, now=now)
    if row is None and 'checkout' not in operations:
        raise HTTPException(404, 'No connected billing record for this account.')
    payment_pending = bool(decision and decision.provider_status == 'active' and decision.access_until is None and not review)
    ...
    return {'phase': 'review' if review else 'awaiting_payment' if payment_pending else 'confirmed' if decision else 'awaiting_confirmation',
            ...
            'can_update_payment_method': bool(linked and not review and decision
                and decision.provider_status in ('active', 'past_due') and payment_method.enabled()),
...
def billing_page(request: Request):
    try:
        ...
        config = json.dumps({'path': PATH, 'csrf': csrf_token(account),
            'paymentCsrf': csrf_token(account, 'payment-method'), 'state': state}).replace('<', '\\u003c')
        return HTMLResponse(PAGE.replace('__CONFIG__', config), headers=HEADERS)
    except FAILURES as error:
        return error_response(error)


@router.post(PATH + '/payment-method')
def open_payment_method(request: Request):
    try:
        account = owner(request)
        if not payment_method.enabled():
            raise HTTPException(404, 'Not found')
        validate_request(request, account, 'payment-method', 'open-payment-method-portal')
        return JSONResponse({'url': payment_method.payment_method_url(account)}, headers=HEADERS)
    except FAILURES as error:
        return error_response(error)
...
function render(s){
 ...
 el('confirmation').textContent=s.billing_review?'Starter access is paused while a refund or payment dispute is reviewed. Contact billing support. You can still cancel renewal below.':s.phase==='awaiting_payment'?'Waiting for payment confirmation for this billing period. Do not make another payment. Refresh or contact billing support.':s.phase==='confirmed'?'Subscription status confirmed by our server.':'Waiting for subscription confirmation. Do not make another payment.';
 ...
 el('payment-controls').hidden=!s.can_update_payment_method;
...
function updateButtons(){el('cancel').disabled=busy||!current.can_cancel||!el('consent').checked;el('refresh').disabled=busy;el('payment-method').disabled=busy||!current.can_update_payment_method;}
...
function schedule(){clearTimeout(timer);if((current.phase==='awaiting_confirmation'||current.phase==='awaiting_payment'||current.cancellation==='awaiting_update')&&polls++<12)timer=setTimeout(refresh,5000);}
async function refresh(){
 if(busy)return;busy=true;updateButtons();
 try{render(await call(config.path+'/status'));el('message').textContent='';el('payment-message').textContent='';schedule();}
 catch(e){current.can_cancel=false;current.can_update_payment_method=false;el('message').textContent=e.message;}
 finally{busy=false;updateButtons();}
}
el('refresh').addEventListener('click',()=>{polls=0;refresh();});
el('consent').addEventListener('change',updateButtons);
el('payment-method').addEventListener('click',async()=>{
 if(busy||!current.can_update_payment_method)return;
 busy=true;clearTimeout(timer);updateButtons();
 try{
  const result=await call(config.path+'/payment-method',{method:'POST',headers:{'X-Billing-CSRF':config.paymentCsrf,'X-Billing-Confirm':'open-payment-method-portal'}});
  const target=new URL(result.url);
  if(target.origin!=='https://buyer-portal.paddle.com'||target.username||target.password)throw new Error('Payment portal is unavailable. Contact billing support.');
  el('payment-message').textContent='Opening Paddle. Review payment details there before confirming.';
  window.location.assign(target.href);
 }catch(e){current.can_update_payment_method=false;el('payment-message').textContent=e.message;}
 finally{busy=false;updateButtons();}
});
