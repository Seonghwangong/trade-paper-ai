...
def authenticated_user_control(user) -> str:
    ...
    return (
        ...
        '<form class="tp-global-search" method="get" action="/search" role="search">'
        '<label class="tp-visually-hidden" for="tp-global-search-input">Global Search</label>'
        '<input id="tp-global-search-input" name="q" type="search" list="tp-global-search-suggestions" '
        'placeholder="Search documents, buyers, products..." autocomplete="off">'
        '<datalist id="tp-global-search-suggestions"></datalist><button type="submit">Search</button></form>'
        ...
        'background:#fff;color:#334155;font:13px Arial,sans-serif}.tp-global-search{display:flex;flex:1;gap:7px;margin:0}.tp-global-search input{width:min(460px,100%);min-height:36px;padding:8px 10px;border:1px solid #CBD5E1;border-radius:9px}.tp-visually-hidden{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.tp-auth-user span{display:grid;gap:2px;text-align:right}.tp-auth-user small{color:#64748B}'
        ...
        '@media(max-width:640px){.tp-auth-user{align-items:stretch;flex-wrap:wrap}.tp-global-search{flex-basis:100%}.tp-global-search input{width:100%}.tp-auth-user span{min-width:0;text-align:left}.tp-auth-user strong,.tp-auth-user small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.tp-auth-user form:not(.tp-global-search){margin-left:auto}}</style>'
        '<script>(function(){const input=document.getElementById("tp-global-search-input"),list=document.getElementById("tp-global-search-suggestions");if(!input||!list)return;let timer;input.addEventListener("input",function(){clearTimeout(timer);const q=input.value.trim();if(!q){list.replaceChildren();return}timer=setTimeout(async function(){try{const response=await fetch("/search-suggestions?q="+encodeURIComponent(q));if(!response.ok)return;const rows=await response.json();list.replaceChildren(...rows.map(function(row){const option=document.createElement("option");option.value=row.value;option.label=row.label;return option}))}catch(error){}},120)})})();</script>'
    )
...
(function(){
  ...
  function confirmDelete(label,archiveMode){
    return new Promise(function(resolve){
      ...
      const verb=archiveMode?'Archive':'Delete';const detail=archiveMode?'You can restore it later from Archive.':'This action cannot be undone.';
      dialog.innerHTML='<h2 id="tp-confirm-title">'+verb+' '+label+'?</h2><p>'+detail+'</p><div class="tp-confirm-actions"><button type="button" class="cancel">Cancel</button><button type="button" class="confirm">'+verb+'</button></div>';overlay.appendChild(dialog);document.body.appendChild(overlay);
  ...
  if(requiredInputs.length){
    ...
    const firstForm=document.querySelector('form[method="post"]');(firstForm||document.body).insertBefore(progress,(firstForm||document.body).firstChild);requiredInputs.forEach(function(field){field.addEventListener('input',updateProgress);field.addEventListener('change',updateProgress);});document.addEventListener('tp:prefill-complete',updateProgress);updateProgress();
  }
  ...
  function itemRecord(row){const name=itemNameField(row);const hs=itemField(row,['[name="hs_code"]','input.hs_code','input[id^="hs"]']);const origin=itemField(row,['[name="origin"]','input.origin','input[id^="origin"]']);const unit=itemField(row,['[name="unit"]','[name="unit_name"]','input.unit','input[id^="unit"]']);const price=itemField(row,['[name="unit_price"]','input.unit_price','input.price','input[id^="price"]']);return {name:String(name&&name.value||'').trim(),hs_code:String(hs&&hs.value||'').trim(),origin:String(origin&&origin.value||'').trim(),unit:String(unit&&unit.value||'').trim(),unit_price:String(price&&price.value||'').trim(),updated:Date.now()};}
  ...
  let productMaster=[];
  function applyMasterProduct(row,product){const mappings=[['name',['[name="item_name"]','input.item','input[id^="item"]']],['hs_code',['[name="hs_code"]','input.hs_code','input[id^="hs"]']],['origin',['[name="origin"]','input.origin','input[id^="origin"]']],['unit',['[name="unit"]','[name="unit_name"]','input.unit','input[id^="unit"]']]];mappings.forEach(function(mapping){const field=itemField(row,mapping[1]);if(!field)return;field.value=String(product[mapping[0]]||'');field.dispatchEvent(new Event('input',{bubbles:true}));field.dispatchEvent(new Event('change',{bubbles:true}));});}
  function enhanceProductMasterRow(row){const field=itemNameField(row);if(!field||field.dataset.tpProductMaster)return;field.dataset.tpProductMaster='true';field.setAttribute('autocomplete','off');const area=document.createElement('div');area.className='tp-product-suggestions';area.setAttribute('role','status');area.setAttribute('aria-live','polite');field.insertAdjacentElement('afterend',area);let timer;
    field.addEventListener('input',function(){window.clearTimeout(timer);timer=window.setTimeout(async function(){const query=String(field.value||'').trim();area.replaceChildren();if(!query)return;let matches=[];try{const response=await fetch('/product-suggestions?q='+encodeURIComponent(query));if(response.ok)matches=await response.json();}catch(error){}matches.forEach(function(product){const button=document.createElement('button');button.type='button';button.className='tp-product-suggestion';button.textContent=String(product.name||'')+(String(product.name||'').toLocaleLowerCase().includes(query.toLocaleLowerCase())?'':' · Did you mean?');button.addEventListener('click',function(){applyMasterProduct(row,product);area.replaceChildren();});area.appendChild(button);});},180);});}
  function enhanceProductMaster(){itemRows().forEach(enhanceProductMasterRow);}
  enhanceProductMaster();document.querySelectorAll('#items_area,.items-container').forEach(function(container){new MutationObserver(enhanceProductMaster).observe(container,{childList:true,subtree:true});});
  ...
  if(workLabel&&activeValidationDefinitions.length){
    ...
    document.addEventListener('tp:prefill-complete',updateSmartValidation);
  ...
  document.addEventListener('submit',async function(event){
    ...
    if(form.method.toLowerCase()==='post'&&action.includes('delete')){
      event.preventDefault();const label=workLabel||document.querySelector('h1')?.textContent?.trim()||'this record';const archiveMode=((event.submitter&&event.submitter.textContent)||'').trim()==='Archive';if(await confirmDelete(label,archiveMode))form.submit();return;
    }
