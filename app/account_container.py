...
def ensure_legacy_container_ownership(container_file, users_file):
    container_file = Path(container_file)
    records = load_json_strict(container_file, [], list)
    if not any(
    ...
    if len(account_ids) != 1:
        raise StorageValidationError(
            "Legacy Container ownership cannot be determined safely."
        )
    ...
    return locked_json_mutation(container_file, [], assign_owner, list)


def public_container(record):
